...
Minimaler Stack, der zeigt wie man eine der fuenf Modul-Varianten in
einem realen Tofu-Projekt einsetzt.

## Was das Beispiel tut

1. Wartet ueber das Modul `01-adminservice-pwsh`, bis `PC123` in ConfigMgr
   den Status "ausgerollt" hat (Task-Sequence Success + Client aktiv).
...
### Variante 03 — WinRM-Jumphost mit Console
```hcl
module "wait_for_pc" {
...
### Variante 05 — WinRM + Cmdlet-Package
```hcl
module "wait_for_pc" {
