...
Linux-Runner ruft die AdminService REST API von MECM direkt mit `Invoke-RestMethod` auf.
...
- `Wait-MecmDeployed.ps1` — Polling-Script, blockiert bis "deployed" oder Timeout
- `main.tf` — Tofu-Beispiel mit `null_resource`

## Aufruf manuell

```bash
pwsh ./Wait-MecmDeployed.ps1 -ComputerName PC123 -SmsProvider sccm.corp.local -SiteCode P01
```
